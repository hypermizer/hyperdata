...
function dependencies(overrides: Partial<StrategyCommandDependencies> = {}): StrategyCommandDependencies {
  return { enabled: true, authenticate: () => Promise.resolve(owner), execute: (_user, command) => Promise.resolve({ type: command.type, id: "result-1" }), ...overrides };
}
...
Deno.test("strategy commands require the exact owner", async () => {
  assertEquals((await handleStrategyCommand(request({}), dependencies({ authenticate: () => Promise.resolve(null) }))).status, 401);
  assertEquals((await handleStrategyCommand(request({}), dependencies({ authenticate: () => Promise.resolve({ id: "x", email: "other@example.com" }) }))).status, 401);
});

Deno.test("disabled command gate rejects before execution", async () => {
  let executed = false;
  const response = await handleStrategyCommand(request({ type: "create_definition", name: "RSI", marginAllocationPct: 10 }), dependencies({ enabled: false, execute: () => { executed = true; return Promise.resolve(); } }));
...
Deno.test("valid create, assignment, pause, and backtest contracts execute", async () => {
  ...
  const commands = [
    ...
    { type: "create_revision", definitionId: uuid, marginAllocationPct: 20 },
...
Deno.test("invalid percentages, assets, date ranges, and identifiers fail closed", () => {
  ...
  assertEquals(parseStrategyCommand({ type: "queue_backtest", revisionId: uuid, assets: ["BTC", "BTC"], start: "2026-07-01", end: "2026-07-02", initialCapital: 5000 }), null);
});
