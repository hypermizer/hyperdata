...
export async function deliverPending(client: SupabaseClient) {
  const { data: claimed, error } = await client.rpc("claim_outbox", { p_limit: 10 });
  ...
  for (const row of (claimed ?? []) as ClaimedOutbox[]) {
    ...
    if (occurrenceError) {
      outcomes.push({ id: row.id, state: "claimed", error: occurrenceError.message });
      continue;
    }
