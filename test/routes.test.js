...
test("site routes resolve top-level and nested paper views", () => {
  assert.deepEqual(parseRoute("#/alerts"), { view: "alerts", paperView: "home", toolsView: "scaling" });
  assert.deepEqual(parseRoute("#/analysis"), { view: "analysis", paperView: "home", toolsView: "scaling" });
  assert.deepEqual(parseRoute("#/principles"), { view: "principles", paperView: "home", toolsView: "scaling" });
  assert.deepEqual(parseRoute("#/paper/order"), { view: "paper", paperView: "order", toolsView: "scaling" });
  assert.deepEqual(parseRoute("#/paper"), { view: "paper", paperView: "home", toolsView: "scaling" });
  assert.deepEqual(parseRoute("#/strats"), { view: "strats", paperView: "home", toolsView: "scaling" });
  assert.deepEqual(parseRoute("#/tools/exposure-ladder"), { view: "tools", paperView: "home", toolsView: "scaling" });
...
test("asset routes preserve the canonical Hyperliquid asset id", () => {
  ...
  assert.deepEqual(parseRoute("#/assets/xyz%3ADRAM/news"), {
    view: "asset", asset: "xyz:DRAM", assetView: "news", interval: "1h", paperView: "home", toolsView: "scaling",
  });
  assert.deepEqual(parseRoute("#/assets/xyz%3ADRAM/financials"), {
    view: "asset", asset: "xyz:DRAM", assetView: "financials", interval: "1h", paperView: "home", toolsView: "scaling",
  });
...
test("unknown routes fall back to the watchlist", () => {
  assert.deepEqual(parseRoute(""), { view: "watchlist", paperView: "home", toolsView: "scaling" });
  assert.deepEqual(parseRoute("#/missing"), { view: "watchlist", paperView: "home", toolsView: "scaling" });
