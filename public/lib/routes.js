  ...
  if (view === "assets" && nestedView) {
    try {
      ...
      if (asset) {
        ...
        return { view: "asset", asset, assetView, interval, analysisView: "trade-log", paperView: "home", toolsView: "scaling" };
      }
    } catch {
      return { view: "watchlist", analysisView: "trade-log", paperView: "home", toolsView: "scaling" };
    }
  }
  if (!SITE_VIEWS.has(view)) return { view: "watchlist", analysisView: "trade-log", paperView: "home", toolsView: "scaling" };
  return {
    view,
    analysisView: view === "analysis" && ANALYSIS_VIEWS.has(nestedView) ? nestedView : "trade-log",
...
export function routeFor(view, nestedView = "home", detailView = "overview", leafView = "1h") {
  ...
  if (view === "analysis") return `#/analysis/${ANALYSIS_VIEWS.has(nestedView) ? nestedView : "trade-log"}`;
